$(function() {
          ...
          $('#dislike').click(function(e) {
                    ...
                    const postId = $(this).attr('data-postId')
                    $.ajax({
                              url: 'http://localhost:8080/api/posts/dislike',
                              type: 'POST',
                              data: {
                                        post_id: postId
                              },
          ...
          function updateReactionsBar(likesCount, dislikeCount)  {
                    const percentage = (parseInt(likesCount) + parseInt(dislikeCount)) === 0 ? 50 : Math.round((parseInt(likesCount) * 100) / (parseInt(likesCount) + parseInt(dislikeCount)))
                    $('.reactions-bar .likes-loader').css('width', `${percentage}%`)
          }

          function updateCount(likesCount, dislikeCount) {
                    $('#likes-count').text(likesCount)
                    $('#dislikes-count').text(dislikeCount)
                    const percentage = (parseInt(likesCount) + parseInt(dislikeCount)) === 0 ? 50 : Math.round((parseInt(likesCount) * 100) / (parseInt(likesCount) + parseInt(dislikeCount)))
                    $('.counts-bar .likesCount-loader').css('width', `${percentage}%`)
                    $('.counts-bar .dislikesCount-loader').css('width', `${100 - percentage}%`)
          }
                                                            ...
                                                            <div class="comment mt-2">
                                                                      <div class="comment-header">
                                                                                ...
                                                                                <div class="user d-flex align-items-center">
                                                                                          <div class="user-avatar mb-1 border">
          ...
          {
                    const {name, body, main} = errors
                    if(name) {
                              const nameEl = $('form #name')
                              nameEl.addClass('is-invalid')
                              const invalidFeedback = `<div class='invalid-feedback'>${name}</div>`
                              nameEl.parent().append(invalidFeedback)
                    }
                    if(body) {
                              const bodyEl = $('form #body')
                              bodyEl.addClass('is-invalid')
                              const invalidFeedback = `<div class='invalid-feedback'>${body}</div>`
                              bodyEl.parent().append(invalidFeedback)
                    }
                    if(main) {
                              $('form').prepend(`<div class='alert alert-danger'>${main}</div>`)
                    }
          }
})